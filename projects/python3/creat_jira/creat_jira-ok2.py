...
# Dependency 列（大小写不敏感）
DEPENDENCY_COLUMN_NAMES = ["dependency", "Dependency", "dependencies", "Dependencies"]
...
def resolve_epic_link_field_id(jira, log):
    """
    解析 'Epic Link' 的字段ID（例如 customfield_10014）。
    在 Jira Server 上 Epic Link 是一个自定义字段。
    """
    name_to_id = build_field_id_map(jira, log)
...
def collect_fields_to_update(row: dict, resolved_ids: dict, jira, project_key: str, log):
    ...
    - 其他 CSV 字段（假定列名已是字段ID或系统字段名；排除 dependency/epic 列）
...
# ---------------- Epic 解析与设置 ----------------
def get_epic_info(row: dict, jira, log):
    """
    若 CSV 提供 epic，则：
      - 强制检查该工单存在；
      - 要求其 issuetype 为 'Epic'；
      - 返回 { 'field_id': epic_link_field_id, 'issue': epic_issue }
    若未提供或为空，返回 None。
    """
    epic_col = find_epic_column(row)
    if not epic_col:
    ...
    raw = (row.get(epic_col) or "").strip()
    if not raw:
        return None

    # 解析 Epic Link 字段ID
    epic_field_id = resolve_epic_link_field_id(jira, log)
    if not epic_field_id:
        raise RuntimeError("未找到 'Epic Link' 字段ID，请联系 Jira 管理员在 Create/Edit 屏幕中添加该字段。")

    # 检查 Epic 工单存在且类型正确
    try:
        epic_issue = jira.issue(raw)
    except Exception as e:
        raise ValueError(f"指定的 Epic 不存在或不可访问：{raw}（{e}）")

    epic_name = getattr(epic_issue.fields.issuetype, "name", "")
    if str(epic_name).strip().lower() != "epic":
        raise ValueError(f"指定的工单不是 Epic 类型：{raw}（实际：{epic_name}）")

    return {"field_id": epic_field_id, "issue": epic_issue}


def set_epic_link(jira, issue, epic_info, log):
    """
    ...
    fid = epic_info["field_id"]
    epic_issue = epic_info["issue"]
    epic_key = epic_issue.key
    epic_id = epic_issue.id

    # 先尝试 key
    try:
        issue.update(fields={fid: epic_key})
    ...
    # 再尝试 id
    try:
        issue.update(fields={fid: epic_id})
...
def resolve_depends_on_link_type(jira, log):
    """
    寻找 'depends on' 链接类型：
    - outward == 'depends on'（忽略大小写）
    - 或 name == 'Dependency' / 'Depends on'
    ...
    for t in types:
        ...
        outward = (getattr(t, "outward", "") or "").strip().lower()
        if outward == "depends on" or name.lower() in ("dependency", "depends on"):
            tid = getattr(t, "id", None)
            return (name or "depends on", tid)
...
def apply_dependency_links(jira, issue_key: str, row: dict, log):
    ...
    for k in keys:
        ...
        try:
            ...
            log(f"[OK] 依赖链接创建：{issue_key} depends on {k}")
        except Exception as e:
...
def create_and_assign(jira: JIRA, row: dict, row_idx: int, resolved_ids, log):
    ...
    # 若提供了 Epic，先强校验（在创建前），避免产生“孤儿”工单
    epic_info = get_epic_info(row, jira, log)
    if epic_info and issuetype.strip().lower() == "epic":
        # 本身就是 Epic，则 Epic Link 不应设置；给个提示但不报错
        log(f"[WARN] 当前工单类型为 Epic，忽略 CSV 中的 epic 设定。")
    ...
    # 设置 Epic Link（如果需要且当前工单不是 Epic）
    if epic_info and issuetype.strip().lower() != "epic":
        set_epic_link(jira, issue, epic_info, log)
...
def main():
    root = tk.Tk()
    root.title("Jira Ticket 批量创建（含 Fix Versions / Dependency / Epic Link）")
